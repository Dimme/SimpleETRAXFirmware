...
{ 
	//trace << "to " << myDestinationAddress << " from " << mySourceAddress << " typeLen " << myTypeLen << endl;

	if (myDestinationAddress == Ethernet::instance().myAddress())
	{
		trace << "to " << myDestinationAddress << " from " << mySourceAddress << " typeLen " << myTypeLen << endl;

		trace << "length " << myLength << " typeLen 0x" << hex << myTypeLen << dec << " (" << myTypeLen << ")" << endl;
		if ((myTypeLen == 0x800) && (myLength > 28) && (*(myData + 20) == 8))
		{
