...
// Create and start the applications.
...
{
	if (myConnection->mySocket) {
		myConnection->mySocket->socketDataSent();
		myConnection->mySocket->socketEof();
	}
...
void TCPSender::sendData(byte* theData, udword theLength)
{
	...
	// Test retransmitt
	// static int c = 0; if (!(++c %= 30)) { delete [] temp; return; }
...
{	
	...
	if (!aConnection) {
		...
		// FIN Received
		if ((aTCPHeader->flags & 0x01) != 0) {
			
			// Check if it is ackompanied by an ACK
			if ((aTCPHeader->flags & 0x10) != 0)
