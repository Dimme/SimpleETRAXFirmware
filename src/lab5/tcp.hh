...
class RetransmissionTimer : public Timed
{
public:
	RetransmissionTimer(TCPConnection * theConnection, Duration theTime);
	void start();
	void cancel();
private:
	void timeOut();
...
public:
	...
	// The sequence number of the first byte in the queue
	udword firstSeq;
	
	// The maximal sequence number that was ever sent
	udword sentMaxSeq;
	...
	// The offered window size from each segment
	udword myWindowSize;
	
	// The retransmission timer for lost segments
	RetransmissionTimer * rTimer;
};
...
public:
	...
	// Handle an incoming SYN segment
	virtual void NetClose(TCPConnection* theConnection, uword withAck);
	// Handle an incoming FIN segment
	...
	virtual void Receive(TCPConnection* theConnection,
		...
		udword theLength);
	// Handle incoming data
	virtual void Acknowledge(TCPConnection* theConnection, udword theAcknowledgementNumber);
	// Handle incoming Acknowledgement
...
public:
	...
	void NetClose(TCPConnection* theConnection, uword withAck);
	// Handle an incoming FIN segment
...
public:
	...
	void Acknowledge(TCPConnection* theConnection, udword theAcknowledgementNumber);
	void Send(TCPConnection* theConnection, byte* theData, udword theLength);
...
class TimeWaitStateTimer : public Timed
{
public:
	TimeWaitStateTimer(Duration theWaitTime, TCPConnection * theConnection);
...
{
public:
	static TimeWaitState* instance(TCPConnection * theConnection);
...
public:
	...
	void sendData(byte* theData, udword theLength);
